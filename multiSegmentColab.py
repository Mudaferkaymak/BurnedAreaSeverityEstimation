...
def main():
        ...
        base_result_path = Path("/content/BurnedAreaSeverityEstimation")
        if not base_result_path.is_dir():
            base_result_path.mkdir(parents=True)
        fold_definition = Path("/content/BurnedAreaSeverityEstimation/satellite_data.csv")

        # master_folder = Path("C:\\Users\\MÜDAFERKAYMAK\\Desktop\\Ara Proje\\Satellite_burned_area_dataset_part1")
        master_folder = Path("/content/drive/MyDrive/SentinelHub")
        ...
        model_class = UNet
        model_args = {'n_classes': 4, 'n_channels': n_channels, 'act': 'relu'}
